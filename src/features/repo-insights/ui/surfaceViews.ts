...
export function surfaceViews(surface: RepoSurfacePayload): SurfaceView[] {
  ...
  return [
    view('api', 'API surface', endpoints.length + routes.length,
    ...
    view('entry', 'Entry points', insights.entryPoints.length,
    ...
    view('map', 'Map', insights.map.files,
    ...
    view('dependencies', 'Dependencies', insights.dependencies.reduce((sum, held) => sum + held.entries.length, 0),
    ...
    view('runtime', 'Runtime', runtimeCount,
    ...
    view('models', 'Data models', insights.models.length,
    ...
    view('tests', 'Tests', insights.tests.caseCount,
    ...
    view('activity', 'Activity', insights.activity?.commits.length ?? null,
...
function view(id: SurfaceViewId, label: string, count: number | null, reason: string, hint: string): SurfaceView {
  return { id, label, count, available: count !== null && count > 0, reason, hint };
}
