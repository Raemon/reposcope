...
import { ownAuthorFilter } from '@/features/github-auth/useViewerLogin';
...
function cachedRepoPulls({ owner, repo }: PullTarget, token: string | null, filters: PullFilters): PullTarget[] {
  ...
  return listedPulls(pulls, filters, ownAuthorFilter(token)).map((pull) => ({ owner, repo, number: pull.number }));
}
...
function cachedAllPulls(token: string | null, filters: PullFilters): PullTarget[] {
  ...
  return listedPulls(found?.pulls ?? [], filters, ownAuthorFilter(token));
}
