...
} satisfies Record<string, ColumnSize>;

type ColumnName = keyof typeof DEFAULTS;

const columnsPref = localPref<Partial<Record<string, ColumnSize>>>('reposcope.columns', {}, decodeColumns);

export function setStickyColumn(name: ColumnName, next: SetStateAction<ColumnSize>, defaultOpen?: boolean): void {
...
export function useStickyColumn(name: ColumnName, defaultOpen?: boolean): [ColumnSize, Dispatch<SetStateAction<ColumnSize>>] {
...
function defaultSize(name: ColumnName, defaultOpen?: boolean): ColumnSize {
  const held = DEFAULTS[name];
