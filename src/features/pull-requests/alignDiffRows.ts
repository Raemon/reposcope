...
export function alignedChangeRows(removed: DiffCell[], added: DiffCell[]): DiffRow[] {
  const rows: DiffRow[] = [];
  let from: Anchor = { left: 0, right: 0 };
  for (const anchor of trimEqualAnchors(removed, added)) {
    zipRows(rows, removed.slice(from.left, anchor.left), added.slice(from.right, anchor.right));
    rows.push(changeRow(removed[anchor.left] ?? null, added[anchor.right] ?? null));
    from = { left: anchor.left + 1, right: anchor.right + 1 };
  }
  zipRows(rows, removed.slice(from.left), added.slice(from.right));
  return rows;
}
...
/** Weighting matches by trimmed length keeps stray `}`/`>` lines from anchoring over real content. */
...
/** Max total weight of a monotone matching of `before[i..]` with `after[j..]`, read as `table[i * stride + j]`. */
function weightTable(before: string[], after: string[]): { table: Uint32Array; stride: number } {
  ...
  for (let i = before.length - 1; i >= 0; i -= 1) {
    for (let j = after.length - 1; j >= 0; j -= 1) {
      ...
      table[i * stride + j] = Math.max(matched, table[(i + 1) * stride + j] ?? 0, table[i * stride + j + 1] ?? 0);
    }
  }
  return { table, stride };
}

function backtrackAnchors(before: string[], after: string[], { table, stride }: { table: Uint32Array; stride: number }): Anchor[] {
  const anchors: Anchor[] = [];
  let i = 0;
  let j = 0;
  while (i < before.length && j < after.length) {
    const weight = matchWeight(before[i], after[j]);
    if (weight > 0 && table[i * stride + j] === weight + (table[(i + 1) * stride + j + 1] ?? 0)) {
      anchors.push({ left: i, right: j });
      i += 1;
      j += 1;
    } else if ((table[(i + 1) * stride + j] ?? 0) >= (table[i * stride + j + 1] ?? 0)) i += 1;
    else j += 1;
  }
  return anchors;
}
