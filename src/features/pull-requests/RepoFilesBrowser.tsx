...
import { browseKey, browsedPath, listedPaths, RepoFileList } from './RepoFileList';
...
function FilesBrowser({ owner, repo, children }: { owner: string; repo: string; children: ReactNode }) {
  ...
  useRegisterColumn(
    'files',
    {
      ...
      onSelect: (item) => setPath(browsedPath(item)),
    },
  ...
  return (
    <div className="flex min-h-0 flex-1 max-md:flex-col max-md:overflow-y-auto">
      ...
      <ResizableColumn navId="files" icon="▤" title="all files" size={fileSize} onSize={setFileSize}>
        <RepoFileList repoFiles={repoFiles} selected={path} onSelect={setPath} query={query} onQuery={setQuery} />
      </ResizableColumn>
